...
class KeywordExtractionTesting():
  ...
  def get_top_n(self,all_keywords,labels,topk=10):
    ...
    cluster_imp_key={}
    print(all_keywords)
    # for data,lbl in zip(all_keywords,labels):
    
    keywords=sorted(all_keywords.items(), key=lambda x: x[1], reverse=True)[:topk]
    print(labels)
    key_labels=[]
    for key in keywords:
      key_labels.append([str(key[0])])
    cluster_imp_key[int(labels[0])]=key_labels
  ...
  def process(self,text):
    ...
    print(f"feature words {feature_words}")
    for ngramword in feature_words:
